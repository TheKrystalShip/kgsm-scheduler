...
public class LeafDescriptorTests
{
    ...
    {
        string path = Path.Combine(RepoRoot(), "deploy", "kgsm-scheduler.env.example");
        ...
        Assert.True(unknown.Count == 0,
            "deploy/kgsm-scheduler.env.example sets these, and kgsm-scheduler.settings.json declares no such key, so " +
